...
This holds on macOS arm64 and Linux x86_64 because both are little-endian and
`mws::core::WavIo` reads/writes WAV with explicit little-endian byte helpers
(no host-endianness assumptions). Float stages are reference-platform-exact and
tolerance-compared elsewhere.
